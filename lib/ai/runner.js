...
async function callClaude(messages, key, model, temperature = 0.3) {
...
async function callOpenAI(messages, key, model, temperature = 0.3) {
    ...
    const payload = isO1
        ...
        : { model: m, messages, max_tokens: 4000, temperature };
    const data = await httpsPost(
...
async function callGrok(messages, key, model, temperature = 0.3) {
    const data = await httpsPost(
        "https://api.x.ai/v1/chat/completions",
        { Authorization: `Bearer ${key}` },
        { model: model || "grok-3", messages, max_tokens: 4000, temperature }
    );
    const text = data?.choices?.[0]?.message?.content;
...
async function callGemini(messages, key, model, temperature = 0.3) {
...
async function callDeepSeek(messages, key, model, temperature = 0.3) {
    const data = await httpsPost(
        "https://api.deepseek.com/chat/completions",
        { Authorization: `Bearer ${key}` },
        { model: model || "deepseek-chat", messages, max_tokens: 4000, temperature }
    );
    const text = data?.choices?.[0]?.message?.content;
