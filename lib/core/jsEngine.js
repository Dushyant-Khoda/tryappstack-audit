...
class JSEngine {
  ...
  auditUnusedPackages() {
    ...
    // Cap each file at 64 KB to avoid unbounded memory growth
    const MAX = 64 * 1024;
    let imports = '';
    for (const f of files) {
      try {
  ...
  auditDeadCode() {
    ...
    // Read all files once — O(n) instead of O(n²)
    const contents = files.map(f => { try { return fs.readFileSync(f, 'utf8'); } catch { return ''; } });
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Check if name appears in combined content of all files
      // (self-references are rare and acceptable as false negatives)
      const occurrences = (combined.match(new RegExp(name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
      if (occurrences <= 1) {
