...
audit_complexity() {
  ...
  while IFS= read -r -d '' f; do
    ...
    [[ $effects -gt 3 ]] && echo "| \`$fp\` | $effects effects | Extract to custom hooks |" && ((issues++))
    local states=$(grep -cE "useState|ref\(|reactive\(|signal\(" "$f" 2>/dev/null || echo 0)
    [[ $states -gt 6 ]] && echo "| \`$fp\` | $states state vars | Use reducer/store |" && ((issues++))
  ...
  [[ $consoles -gt 0 ]] && echo "| *$consoles files* | console.log leftovers | Remove before shipping |" && ((issues++))
  ...
  if $IS_TYPESCRIPT; then
    ...
    [[ $any_total -gt 0 ]] && echo "| *codebase* | $any_total \`any\` usages | Add proper types |" && ((issues++))
  fi

  # .then chains
  local chains=$(find "$src" -type f \( -name '*.ts' -o -name '*.js' \) -not -name '*.test.*' \
    -exec grep -c '\.then(' {} + 2>/dev/null | awk -F: '$NF>3{sum++} END{print sum+0}')
  [[ $chains -gt 0 ]] && echo "| *$chains files* | .then() chains | Use async/await |" && ((issues++))
