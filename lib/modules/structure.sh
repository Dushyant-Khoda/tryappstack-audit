...
audit_structure() {
  ...
  ck_fail() { echo "| 🔴 | $1 |"; (( checks++ )) || true; (( issues++ )) || true; }
  ...
  while IFS= read -r f; do
    ...
    if [[ $c -ge 3 && ! -f "$f/index.ts" && ! -f "$f/index.tsx" && ! -f "$f/index.js" ]]; then
      ck_warn "${f#$dir/} — $c files, no barrel export"; (( barrel_miss++ )) || true
    fi
  ...
  while IFS= read -r -d '' f; do
    ...
    [[ $depth -gt 5 ]] && { ck_warn "Deep nesting: ${f#$dir/}"; (( deep++ )) || true; }
