...
/**
 * Check if bash 4+ is available on the system.
 */
function hasBash() {
...
function loadAIConfig(opts = {}) {
    ...
    if (!key && process.env.XAI_API_KEY) {
...
function sanitizeForAI(content) {
    return content
        ...
        .replace(
            /\b[A-Z][A-Z0-9_]*(?:KEY|SECRET|TOKEN|PASSWORD|PASS|PWD|API|AUTH)\s*=\s*[^\n]+/g,
            "[REDACTED]"
        )
