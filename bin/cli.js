...
const BRAND_BG = chalk.bgHex('#c8ff00').black;
...
program
  ...
  .option('--strict [threshold]', 'CI mode — exit 1 if score < threshold', '70')
  .option('--ai', 'Enable AI-powered insights')
  .option('--ai-key <key>', 'AI API key for this run')
  .option('--ai-provider <provider>', 'AI provider: claude | openai | grok | gemini | deepseek', 'claude')
...
program
  .command('init')
  .description('Setup .auditrc config + git pre-push hook')
  .action(runInit);
...
program
  .command('insights')
  .description('Hidden issue finder — security, API quality, architecture red flags (AI-enhanced if key set)')
  .argument('[directory]', 'Project directory', '.')
...
program
  .command('codedocs')
  .description('Generate complete project documentation (routes, components, models, env vars, AI architecture analysis)')
  .argument('[directory]', 'Project directory', '.')
...
program
  .command('bizplan')
  .description('AI Revenue Analyst — monetization strategies, market trends, tool ROI (AI required)')
  .argument('[directory]', 'Project directory', '.')
...
program
  .command('features')
  .description('AI Business Analyst — feature gap analysis, market comparison, revenue prioritisation (AI required)')
  .argument('[directory]', 'Project directory', '.')
...
program
  .command('estimate')
  .description('AI Project Manager — interactive sprint timeline with story points and cost estimate (AI required)')
  .argument('[directory]', 'Project directory', '.')
  .option('--ai-key <key>', 'AI API key for this run')
  .option('--ai-provider <provider>', 'AI provider: claude | openai | grok | gemini | deepseek')
  .action((directory, opts) => {
...
program
  .command('testplan')
  .description('AI Tester — test strategy, test cases per route/component, CI config (AI required)')
  .argument('[directory]', 'Project directory', '.')
  .option('--ai-key <key>', 'AI API key for this run')
  .option('--ai-provider <provider>', 'AI provider: claude | openai | grok | gemini | deepseek')
  .action((directory, opts) => {
...
program
  .command('brand')
  .description('AI Marketing Strategist — ICP, GTM strategy, ready-made AI prompts for launch (AI required)')
  .argument('[directory]', 'Project directory', '.')
  .option('--ai-key <key>', 'AI API key for this run')
  .option('--ai-provider <provider>', 'AI provider: claude | openai | grok | gemini | deepseek')
  .action((directory, opts) => {
...
program
  .command('legal')
  .description('Legal & compliance audit — GDPR, CCPA, Privacy Policy, ToS requirements (AI-enhanced if key set)')
  .argument('[directory]', 'Project directory', '.')
  .option('--ai-key <key>', 'AI API key for this run')
  .option('--ai-provider <provider>', 'AI provider: claude | openai | grok | gemini | deepseek')
  .action((directory, opts) => {
...
program
  .command('context')
  .description('Generate .tas-context.md — compressed project context for any AI chat (90% fewer tokens than pasting files)')
  .argument('[directory]', 'Project directory', '.')
...
program
  .command('team')
  .description('AI Team Member REPL — @dev · @architect · @security · @qa · @pm write & review code (AI required)')
  .argument('[directory]', 'Project directory', '.')
  .option('--ai-key <key>', 'AI API key for this run')
  .option('--ai-provider <provider>', 'AI provider: claude | openai | grok | gemini | deepseek')
  .option('--ai-model <model>', 'Override model for this run')
  .option('--temperature <n>', 'Override temperature 0-2 (default: from config or 0.3)', parseFloat)
  ...
  .action(() => {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (provider) {
          console.log(`  ${chalk.green('✓')}  AI configured — ${chalk.cyan(provider)} · ${chalk.cyan(model || 'default')}`);
        }
      } catch { /* ignore */ }
    } else {
      console.log(`  ${chalk.dim('–')}  AI not configured — run ${BRAND('tas-audit ai-setup')}`);
    }
...
async function runAudit(directory, opts) {
  ...
  // Banner (skip in pre-push)
  if (!opts.prePush) {
    printBanner();
  }

  // Detect hasBash
  const hasBash = os.platform() !== 'win32' && shellExists('bash');
  const hasWSL = os.platform() === 'win32' && shellExists('wsl');

  if (!hasBash && !hasWSL) {
    // Use pure JS engine as fallback
    runJSAudit(dir, opts);
    return;
  }
...
async function runAISetup() {
  ...
  console.log(chalk.dim('  AI analyzes your audit scores and gives specific refactoring advice.'));
  ...
  if (!key) { console.log(chalk.red('  No key provided.')); return; }
...
function runDoctor() {
  ...
  for (const [name, cmd, required, flag] of checks) {
    try {
      const ver = execSync(`${cmd} ${flag} 2>/dev/null`, { encoding: 'utf8' }).trim().split('\n')[0].slice(0, 40);
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.log('\n  ' + chalk.yellow('○') + ' AI not configured — run: ' + BRAND('tryappstack-audit ai-setup'));
  }

  // tryappstack check
  try {
    execSync('npx tryappstack --version 2>/dev/null', { encoding: 'utf8' });
    console.log('  ' + chalk.green('✓') + ' tryappstack (boilerplate CLI)');
  } catch {
    console.log('  ' + chalk.yellow('○') + ` tryappstack — ${chalk.dim('npm i -g tryappstack for boilerplates')}`);
  }
...
function runCompare(file1, file2) {
  ...
  for (const mod of allMods) {
    const a = s1[mod] || 0, b = s2[mod] || 0, diff = b - a;
  ...
  console.log('  ' + (totalDiff > 0 ? chalk.green.bold(`↑ +${totalDiff} improvement`) : totalDiff < 0 ? chalk.red.bold(`↓ ${totalDiff} regression`) : chalk.dim('No change')) + '\n');
}

// ── Helpers ──

function shellExists(cmd) {
  try { execSync(`which ${cmd} 2>/dev/null`, { stdio: 'ignore' }); return true; } catch { return false; }
}
...
function printBanner() {
  ...
  if (require('fs').existsSync(configPath)) {
    try {
      ...
      const icons = { claude:'🟣', anthropic:'🟣', openai:'🟢', gpt:'🟢', grok:'🔵', xai:'🔵', gemini:'🟡', google:'🟡', deepseek:'🔴' };
      const names = { claude:'Claude', anthropic:'Claude', openai:'GPT', gpt:'GPT', grok:'Grok', xai:'Grok', gemini:'Gemini', google:'Gemini', deepseek:'DeepSeek' };
      if (p) {
        const tag = BRAND('◆ AI Connected') + chalk.dim(` ${icons[p]||'🤖'} ${names[p]||p}${m ? ' · ' + m : ''}`);
...
function printAIComparison() {
  console.log('  ┌──────────────────────────────────────────────────────────────┐');
  console.log('  │  ' + chalk.bold('Without AI') + '                  │  ' + BRAND.bold('With AI ✨') + '                    │');
  console.log('  ├──────────────────────────────────────────────────────────────┤');
  ...
  console.log('  │  ' + chalk.dim('Free forever') + '                │  ' + chalk.dim('~$0.01/run, your key') + '       │');
  console.log('  └──────────────────────────────────────────────────────────────┘');
  console.log('');
...
function runTrend(directory) {
  ...
  if (files.length >= 2) {
    ...
    const last = extractScore(fs.readFileSync(path.join(auditsDir, files[files.length - 1]), 'utf8')) || 0;
    const total = last - first;
    console.log('\n  ' + chalk.bold('Overall: ') + (total >= 0 ? chalk.green.bold(`↑ +${total}`) : chalk.red.bold(`↓ ${total}`)) + chalk.dim(` (${files[0]} → ${files[files.length - 1]})`));
  }
  console.log('');
}
...
function runWatch(directory, opts) {
  ...
  const runOnce = () => {
    ...
    try {
      execSync(`node "${path.join(__dirname, 'cli.js')}" "${dir}" -n "watch-latest"`, { stdio: 'inherit', cwd: dir });
    } catch {}
  };
  ...
  try {
    fs.watch(srcDir, watchOpts, (event, filename) => {
      if (!filename) return;
      if (filename.includes('node_modules') || filename.includes('.git') || filename.includes('audits')) return;
...
// ── JS Engine Fallback (Windows without bash) ──
function runJSAudit(dir, opts) {
  ...
  const outputFile = path.join(outputDir, `audit-${timestamp}.md`);
...
`;
  ...
  // JSON output
  if (opts && opts.json) {
    console.log(JSON.stringify({ project, framework: result.framework.display, score: avg, grade, modules: result.scores }, null, 2));
  }

  // Strict check
  if (opts && opts.strict) {
    const threshold = parseInt(opts.strict) || 70;
    if (avg < threshold) {
      console.log(chalk.red.bold(`  ✗ FAILED: ${avg} < ${threshold}`));
      process.exit(1);
    }
  }
}
